...
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.ALARM;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_ALARMSSET;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_ALARM_TYPE;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_BOTTLECOLOR;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_CUSTOM_ALARM_URI;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_FREQUENCY;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_ISTAKEN;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_START_DATE;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_STOCKUP;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_SUPPLY;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_TIME;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_TIMETAKEN;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.COLUMN_TITLE;
import static com.quangthe.nhacnho_uongthuoc.DatabaseHelper.CUSTOM_ALARM;
...
public class Pill {
    ...
    public static final Uri DEFAULT_ALARM_URI =
            Uri.parse("android.resource://com.quangthe.nhacnho_uongthuoc/" + R.raw.eas_alarm);
    ...
    public boolean isDoseTaken(int index) {
        return (taken & (1 << index)) != 0;
    }

    public void setDoseTaken(int index, boolean isTaken) {
        if (isTaken) {
            taken |= (1 << index);
        } else {
            taken &= ~(1 << index);
        }
        contentValues.put(DatabaseHelper.COLUMN_ISTAKEN, taken);
    }

    public void resetPill(Context context, int recyclerViewPosition) {
        int bitsSet = Integer.bitCount(taken);
        if (getSupply() >= 0) {
            setSupply(getSupply() + bitsSet);
        }
                ...
                new NotificationCompat.Builder(context, Simpill.PILL_REMINDER_CHANNEL)
                        ...
                        .setContentText(
                                context.getString(
                                        ...
                                        doseIndex < getTimesArray().length ? getTimesArray()[doseIndex] : getName()))
                        .setColor(500086)
                        .setStyle(
                                new NotificationCompat.BigTextStyle()
                                        .bigText(
                                                context.getString(
                                                        R.string.reminder_notification_description,
                                                        getName() + " (" + (doseIndex < getTimesArray().length ? getTimesArray()[doseIndex] : "") + ")")))
    ...
    public void deletePillFromDatabase(Context context, int recyclerViewPosition) {
        ...
        new DatabaseHelper(context).softDeletePill(this);
        PillListener pillListener = (PillListener) context;
