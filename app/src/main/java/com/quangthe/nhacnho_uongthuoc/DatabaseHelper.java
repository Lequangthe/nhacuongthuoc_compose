...
import static com.quangthe.nhacnho_uongthuoc.ArrayHelper.STR_SEPARATOR;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Cursor readSqlDatabase() {
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_ISDELETED + " = 0";
        SQLiteDatabase myDatabase = this.getReadableDatabase();
    ...
    public Cursor readTrashDatabase() {
        String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_ISDELETED + " = 1";
        SQLiteDatabase myDatabase = this.getReadableDatabase();
    ...
    public Pill[] getTrashPills() {
        Cursor cursor = readTrashDatabase();
    ...
    public void softDeletePill(Pill userPill) {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] selectionArgs = new String[] {String.valueOf((userPill.getPrimaryKey()))};
        android.content.ContentValues cv = new android.content.ContentValues();
        cv.put(COLUMN_ISDELETED, 1);
        db.update(TABLE_NAME, cv, SELECTION, selectionArgs);
    }

    public void restorePill(Pill userPill) {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] selectionArgs = new String[] {String.valueOf((userPill.getPrimaryKey()))};
        android.content.ContentValues cv = new android.content.ContentValues();
        cv.put(COLUMN_ISDELETED, 0);
        db.update(TABLE_NAME, cv, SELECTION, selectionArgs);
    }
